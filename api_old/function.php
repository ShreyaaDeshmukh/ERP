   ...
   <body style="background-color: #fff;">
      ...
      <table border="0" cellpadding="0" cellspacing="0" width="100%">
		...
		function multipleAddress($con,$json){
			...
			// new code inserted for multiple address
             $query =  'select * from tbl_customer_address where customer_id="'.$customer_id.'"';
			// // echo $query;die;
			 $result = mysqli_query($con, $query);

			 $query1 = 'Select * from customer_ship_to where customer_id="'.$customer_id.'"';
			 ...
			 if(!empty($result)){
				...
				if(!empty($result1)){
					...
					//jdksaj
					$values = mysqli_fetch_all($result1);
					
					
					 
					for($i=0;$i<$rows;$i++){
						
						array_push($a,array('id'=>$values[$i][0],'customer_id'=>$values[$i][0],'address'=>$values[$i][6],'timestamp'=>$values[$i][7]));

					}	  
					$response['data'] = array("status"=>"true", "address"=>$a);
						return $response;
					  }
					  else{
						...
						$values = mysqli_fetch_assoc($result1);
							...
							array_push($a,array('id'=>$values['id'],'customer_id'=>$values['customer_id'],'address'=>$values['ship_to_address'],'timestamp'=>$values['created_at']));
				...
				//end of new code
				 ...
				 $response['data'] = array("status"=>"false", "address"=>[]);
				return $response;
			 }
