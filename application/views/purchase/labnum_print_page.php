...
<script type="text/javascript">
	function printDiv(divName) {
		...
		// printContents = '<html><head><style>height: "300px"</style></head><body>'+printContents+'</body></html>';
    ...
    mywindow.document.write('<html><title>Print</title><body>'+content+'</body></html>');
...
function printfew(){
	...
	for(let j=0; j< arr.length; j++){
		console.log(arr[j]);
		if(document.getElementById(arr[j]) != null){
			var  content = document.getElementById(arr[j]).innerHTML;
	...
	setTimeout(function(){
		...
		mywindow.document.write('<html><title>Print</title><body>'+contentAll+'</body></html>');
                    ...
                    <div class="panel-body">
						...
						?>
							<div style="float: center">
								...
								<a  class="btn btn-danger" href="<?php echo base_url('Crecieving/manage_recieving/'.$r_id);?>"><?php echo display('cancel')?></a>
								<a style="float:right;" class="btn btn-info" href="#" onclick="printfew()">Print few</a>
							</div>
						<?php
						}
						?>
                        <div class="" style="margin-top: 10px">
							...
							?>
								<div id="printableArea">
									<?php $i=0;
									 foreach($labnum_data as $labnum): 
										$i++;
									
									#for($i=1;$i<=$labnum['quantity'];$i++):
									//$imsg  = $controller->barcode_generator($labnum['label']);
									?>
									<div class="row" id = <?php echo $i; ?> style="border:1px solid;">	
								<input type="checkbox" onclick="boxDisable(<?php echo $i; ?>, $(this));" >
									<div class="col-md-12" style="width:100%; height:100%;">

											<center>
											<div><?php echo "<b>Product Name</b> : ". $labnum['product_name'];?></div>
											<div><?php echo "<b>Product Description</b> : ". $labnum['product_details'];?></div>
											<?php $arr = explode("###",$labnum['unit']);

											?>
											<div><?php echo "<b>Unit</b> : ". $arr[0];?></div>
											<div><?php echo "<b>Serial Number</b> : ". $labnum['serial_number'];?></div>
											<div><?php echo "<b>Expiry Date</b> : ". $labnum['expiry_date'];?></div>
											<div><?php echo "<b>LOT</b> : ". $labnum['lot'];?></div>
											<div><?php echo "<b>Each Quantity</b> : ". $labnum['total_quantity'];?></div>
											...
											<div>&nbsp;</div>
											<div>
												<img src="<?php echo base_url('Cbarcodepurchase/barcode_generator/'.$labnum['label'])?>" class="img-responsive center-block" alt="" style="display: block;margin-left: auto;margin-right: auto;">
											</div>	
											<div>&nbsp;</div>
