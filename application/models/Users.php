<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Users extends CI_Model {

    public function __construct() {
        parent::__construct();
    }

    function check_valid_user($username, $password) {
        $password = $password;	
        $this->db->where(array('l_email' => $username, 'l_password' => $password));
        $query = $this->db->get('register_login');
        $result = $query->result_array();
     
        if (count($result) == 1) {
            $user_id = $result[0]['l_id'];
            // print_r($user_id);exit;	
           
            $this->db->select('*');
            $this->db->from('register_login');
            // $this->db->join('users b', 'b.user_id = a.user_id');
            $this->db->where('l_id', $user_id);
            $query = $this->db->get();
            // echo "this is....";
            // print_r($query->result_array());exit;
            return $query->result_array();
        }
        return false;
    }



    function login($username, $password) {
        $password = $password;  
        $this->db->where(array('username' => $username, 'password' => $password));
        $query = $this->db->get('user_login');
        return $query->result_array();
     
       
    }


    /*
     * *User registration
     */
   
    public function user_registration() {
        $birth_day = $this->input->post('birth_day');
        $birth_month = $this->input->post('birth_month');
        $birth_year = $this->input->post('birth_year');
        $dbo = $birth_year . '-' . $birth_month . '-' . $birth_day;

        $data1 = array(
            'user_id' => null,
            'first_name' => $this->input->post('first_name'),
            'last_name' => $this->input->post('last_name'),
            'gender' => $this->input->post('user_sex'),
            'date_of_birth' => $dbo,
            'status' => 1
        );
        $this->db->insert('users', $data1);
        $insert_id = $this->db->insert_id();
        //Inset user Login table 

        $password = $this->input->post('password');
        $password = md5("ctgs" . $password);

        $data = array(
            'user_id' => 1, //$insert_id ,
            'username' => $this->input->post('username'),
            'password' => $password,
            'user_type' => 2,
            'security_code' => '',
            'status' => 0
        );
        $this->db->insert('user_login', $data);
    }

    public function profile_edit_data() {
        $user_id = $this->session->userdata('user_id');
        $this->db->select('a.*,b.username');
        $this->db->from('users a');
        $this->db->join('user_login b', 'b.user_id = a.user_id');
        $this->db->where('a.user_id', $user_id);
        $query = $this->db->get();

        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
        return false;
    }

    //Update Profile
    public function profile_update() {
        $this->load->library('upload');

        if (($_FILES['logo']['name'])) {
            $files = $_FILES;
            $config = array();
            $config['upload_path'] = 'assets/dist/img/profile_picture/';
            $config['allowed_types'] = 'gif|jpg|png|jpeg|JPEG|GIF|JPG|PNG';
            $config['max_size'] = '1000000';
            $config['max_width'] = '1024000';
            $config['max_height'] = '768000';
            $config['overwrite'] = FALSE;
            $config['encrypt_name'] = true;

            $this->upload->initialize($config);
            if (!$this->upload->do_upload('logo')) {
                $sdata['error_message'] = $this->upload->display_errors();
                $this->session->set_userdata($sdata);
                redirect('Admin_dashboard/edit_profile');
            } else {
                $view = $this->upload->data();
                $logo = base_url($config['upload_path'] . $view['file_name']);
            }
        }

        $old_logo = $this->input->post('old_logo');

        $user_id = $this->session->userdata('user_id');
        $first_name = $this->input->post('first_name');
        $last_name = $this->input->post('last_name');
        $user_name = $this->input->post('user_name');
        $new_logo = (!empty($logo) ? $logo : $old_logo);

        return $this->db->query("UPDATE `users` AS `a`,`user_login` AS `b` SET `a`.`first_name` = '$first_name', `a`.`last_name` = '$last_name', `b`.`username` = '$user_name',`a`.`logo` = '$new_logo' WHERE `a`.`user_id` = '$user_id' AND `a`.`user_id` = `b`.`user_id`");
    }

    //Change Password
    public function change_password($email, $old_password, $new_password) {
        $user_name =$new_password;
        $password = $old_password;
        $this->db->where(array('username' => $email, 'password' => $password));
        $query = $this->db->get('user_login');
        // print_r($this->db->last_query());die;
        $result = $query->result_array();
        
        if (count($result) == 1) {
            $this->db->set('password', $user_name);
            $this->db->where('password', $password);
            $this->db->where('username', $email);
            $this->db->update('user_login');

            return true;
        }
        return false;
    }
    function check_signupMe($userdata_check){
        $this->db->select('*');
        $this->db->from('user_login');
        $this->db->where('username',$userdata_check['username']);
       $query = $this->db->get();
    //  echo $query->num_rows();exit();
        return $query->num_rows();
}
function signupMe($userdata)
{
    // $username = $userdata['r_email'];
    // $password = $userdata['r_password'];
    // print_r ($password);
    // die();
    $this->db->trans_start();
    $this->db->insert('register_user', $userdata);


    $data = array(
        'user_id' => $userdata['user_id'], //$insert_id ,
        'username' =>$userdata['r_email'],
        'password' => $userdata['r_password'],
        'reg_time' => $userdata['r_date_time'],
        'subscription' => $userdata['r_subscription'],
        'databaseName' => 'wholesale_subscription_test',
        'user_type' => 1,
        'security_code' => '',
        'status' => 0
    );
    $this->db->insert('user_login', $data);
    
    $insert_id = $this->db->insert_id();        
    $this->db->trans_complete();        
    return $insert_id;
}
function loginMe($user_login)
{ 
     $this->db->select('*');
    $this->db->from('user_login');
    $this->db->where($user_login);
    $query = $this->db->get();
	
	// print_r($query);die;
    $query->num_rows();
    // print_r($query->num_rows());
    return $query->num_rows();
    
}
function check_login($user_login1)
{
     $this->db->select('*');
    $this->db->from('user_login');
    $this->db->where($user_login1);
    $query = $this->db->get();
    // echo $query->num_rows();
    return $query->num_rows();
    
}
function check_email($email){
    $this->db->select('*');
    $this->db->from('user_login');
    $this->db->where('username',$email);
    $query = $this->db->get(); 
    return $query->result();
    // print_r($query->result());exit;


}
function login_insert($user_login){
  
    $user_login_data=array(
   "username"=>$user_login->r_email , 
   "password"=> $user_login->r_password,
   "lic_key"=>$user_login->licence_session, 
   "subscription"=>$user_login->r_subscription,
   "reg_time"=>$user_login->r_date_time,

   );
   $user_exist =array(
       "username"=>$user_login->r_email , 
       "lic_key"=>$user_login->licence_session,
   );
   $this->db->select('*');
   $this->db->from('user_login');
   $this->db->where($user_exist);
   $query = $this->db->get();
   // print_r($query->num_rows());die;       
       if($query->num_rows()>0){           
           return false;
       }
       else{
         
           $this->db->insert('user_login', $user_login_data);
           $insert_id = $this->db->insert_id();        
           $this->db->trans_complete();        
           return $insert_id;
       }
   
}

#==================reset password=============================
 public function updatePassword($email,$password,$key){
	 $this->db->set('password', $password); //value that used to update column  
     $this->db->where('username', $email); //which row want to upgrade 
	 $this->db->where('lic_key', $key);
     $this->db->update('user_login');
	 
     return $email;
	 
 }


}
