...
<div class="content-wrapper">
    <section class="content-header">
        ...
        <div class="header-title">
            <h1><?php echo display('receipt_edit') ?></h1>
            <small><?php echo display('receipt_edit') ?></small>
            <ol class="breadcrumb">
                <li><a href="#"><i class="pe-7s-home"></i> <?php echo display('home') ?></a></li>
                <li><a href="#"><?php echo display('receipt') ?></a></li>
                <li class="active"><?php echo display('receipt_edit') ?></li>
            </ol>
        </div>
    </section>
                ...
                <div class="panel panel-bd lobidrag">
                    <div class="panel-heading">
                        <div class="panel-title">
                            <h4><?php echo display('receipt_edit') ?></h4>
                        </div>
                    </div>
                   <?php echo form_open_multipart('Creceipt/receipt_update',array('class' => 'form-vertical', 'id' => 'receipt_update'))?>
                        ...
                        <div class="row table-responsive">
                            ...
                            <tbody>
                              <tr>
                                <td>
                                    <?php //$date = date('Y-m-d'); ?>
                                        <input type="text" tabindex="2" class="form-control datepicker" name="receipt_date" value="{receipt_date}" id="receipt_date" required  tabindex='2'/>
                                    </td>
                                <td>
 ...
 <td style="width: 200px;text-align: center;">
    <input type="text" name="product_name" onclick="receipt_productList({sl});" value="{product_name}-({product_model})" required class="form-control productSelection" placeholder='Type your Product Name' id="product_name_{sl}" >
...
<input type="hidden" name="cartoon[]" onkeyup="quantity_calculate({sl});" onchange="quantity_calculate({sl});" value="{cartoon}" required  id="qty_item_{sl}" class="form-control text-right" min="0" />

<input type="hidden" name="cartoon_item[]" value="{cartoon_quantity}" readonly="readonly" id="ctnqntt_{sl}" class="ctnqntt{sl} form-control text-right" />
<td class="text-right">
    <input type="text" name="product_quantity[]" value="{quantity}" id="total_qntt_{sl}" class="form-control qty_calculator text-right" />
</td>
<td>
...
<input type="hidden" name="receipt_detail_id[]" value="{receipt_detail_id}"/>
 ...
 <td>
    <button style="text-align: right;" class="btn btn-danger" type="button" value="Delete" onclick="deleteRow(this)" tabindex="7"><?php echo display('delete')?></button>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <input type="submit" id="add-receipt" class="btn btn-success btn-large" name="add-receipt" value="<?php echo display('save_changes') ?>" tabindex="8" />
                            </div>
                        </div>
                    </div>
                    <?php echo form_close()?>
