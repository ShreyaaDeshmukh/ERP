...
class Cinventory extends CI_Controller {
    ...
    public function manage_product($r_id) {
        ...
        $config["base_url"] = base_url('Cinventory/manage_product/'.$r_id);
        $config["total_rows"] = $this->Products->product_list_count_inventory($r_id);
        $config["per_page"] = 10;
        ...
        #pagination ends
        #  
        $content = $this->linventory->product_list($links, $config["per_page"], $page,$r_id);

        $this->template->full_admin_html_view($content);
    }
}
	//Product Add Form
    public function manage_checkout($r_id) {
        $CI = & get_instance();
        // $this->auth->check_admin_auth();
        $CI->load->library('linventory');
        $CI->load->model('Products');
        ...
        $config["base_url"] = base_url('Cinventory/manage_checkout/'.$r_id);
        $config["total_rows"] = $this->Products->product_list_count_checkout($r_id);
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        /* This Application Must Be Used With BootStrap 3 * */
        ...
        #  
        // echo "hello";
        $content = $this->linventory->product_checkout_list($links, $config["per_page"], $page, $r_id);
	    ...
	    public function manage_checkin($r_id) {
        $CI = & get_instance();
        // $this->auth->check_admin_auth();
        $CI->load->library('linventory');
        $CI->load->model('Products');
        ...
        $config["base_url"] = base_url('Cinventory/manage_checkin/'.$r_id);
        $config["total_rows"] = $this->Products->product_list_count_checkin($r_id);
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        /* This Application Must Be Used With BootStrap 3 * */
        ...
        #  
        // echo "hello";
        $content = $this->linventory->product_checkin_list($links, $config["per_page"], $page, $r_id);
    ...
    public function insert_product() { #echo "<pre>";print_r($_POST);die;
        $CI = & get_instance();
        $CI->auth->check_admin_auth();
        $CI->load->library('lproduct');
        ...
        */
        ...
        if ($result == 1) {
            ...
            if (isset($_POST['add-product'])) {
                redirect(base_url('Cinventory/manage_product'));
                exit;
    ...
    public function product_update_form($product_id) {
        ...
        $content = $CI->linventory->product_details_inventory($product_id);
        $this->template->full_admin_html_view($content);
    }
	}
    // Product Update
...
//        echo "SX".$old_image; die();
        ...
        if($productPerCarton/$innerCartonPerCarton!=$productPerInnerCarton){
            $error = array('error' => $this->upload->display_errors());
            $this->session->set_userdata(array('error_message' => 'InnerCarton Unit mismatch with regards to carton, Please check again!'));
            redirect(base_url('Cinventory/product_update_form/'.$product_id));
        }
...
//        echo '<pre>';        print_r($data);        die();
    ...
    public function product_by_search() {
		...
		$content = $CI->linventory->product_search_list($product_id);
        $sub_menu = array(
