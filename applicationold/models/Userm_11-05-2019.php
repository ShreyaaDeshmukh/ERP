...
class Userm extends CI_Model {
    ...
    public function user_entry($data) {
			...
			if(isset($CI->session->userdata['databaseName'])){
				$dbname = $CI->session->userdata['databaseName'];
			}
			...
			if($dbname == "wholesale_v3"){
					
			}elseif($dbname == "wholesale_vn1"){
				$this->db->query("call STR_addUser('".$dbname."','".$data['user_id']."')");
			}
